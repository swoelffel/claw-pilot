...
export class DiscoverDialog extends DialogMixin(LitElement) {
  ...
  private async _adopt(): Promise<void> {
    ...
    try {
      ...
      setTimeout(() => {
        this.dispatchEvent(new CustomEvent("instances-adopted", {
    ...
    return html`
      <div class="instance-card">
        ...
        <div class="instance-meta">
          ...
          ${inst.telegramBot
            ? html`<span class="meta-telegram">✈ ${inst.telegramBot}</span>`
            : ""}
          ${inst.defaultModel
            ? html`<span class="meta-model">${inst.defaultModel}</span>`
            : ""}
          ${inst.agentCount > 0
            ? html`<span class="meta-agents">${inst.agentCount} agent${inst.agentCount !== 1 ? "s" : ""}</span>`
            : ""}
        </div>
      </div>
    `;
      ...
      case "scanning":
        return html`
          <div class="spinner-state">
            ...
            <div class="spinner-hint">${msg("Looking for OpenClaw instances", { id: "discover-scanning-hint" })}</div>
          </div>
        `;
          ...
          return html`
            <div class="dialog-body">
              <div class="empty-results">
                <p>${msg("No OpenClaw instances found on this system.", { id: "discover-none-found" })}</p>
                <p>Make sure OpenClaw is installed and at least one instance directory exists.</p>
              </div>
            </div>
            <div class="dialog-footer">
              <button class="btn btn-ghost" @click=${this._close}>${msg("Close", { id: "btn-cancel" })}</button>
            </div>
          `;
        }
        return html`
          <div class="dialog-body">
            <div class="found-label">${msg(`Found ${this._found.length} instance(s) on this system:`, { id: "discover-found-n" })}</div>
          ...
          <div class="dialog-footer">
            ...
            <button
              ...
              @click=${() => void this._adopt()}
            >${msg(`Adopt all (${this._found.length})`, { id: "discover-adopt-btn" })}</button>
          </div>
        `;
      ...
      case "done":
        return html`
          <div class="done-state">
            ...
            <div>${msg(`${this._adoptedCount} instance(s) registered successfully.`, { id: "discover-done" })}</div>
          </div>
        `;
  ...
  override render() {
    return html`
      <div
        ...
        @click=${(e: Event) => { if (e.target === e.currentTarget && this._isCloseable()) this._close(); }}
      >
        <div class="dialog">
