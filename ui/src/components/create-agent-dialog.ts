...
export class CreateAgentDialog extends DialogMixin(LitElement) {
  ...
  private _validateSlug(value: string): string {
    ...
    if (!/^[a-z][a-z0-9-]*$/.test(value))
      return msg("ID must be 2-30 lowercase chars", { id: "cad-error-slug-invalid" });
    if (value.length < 2 || value.length > 30)
      return msg("ID must be 2-30 lowercase chars", { id: "cad-error-slug-invalid" });
    if (this.existingAgentIds.includes(value))
      return msg("This ID is already used", { id: "cad-error-slug-taken" });
    return "";
  }
  ...
  private _renderSpinner() {
    return html`
      <div class="spinner-overlay">
        ...
        <div>
          ${msg("Creating agent", { id: "cad-spinner-creating" })}
          <strong>${this._agentSlug}</strong>...
        </div>
      </div>
    `;
  }
    ...
    return html`
      <div class="dialog-body">
        ...
        <div class="section">
            ...
            <div class="field">
              ...
              ${this._slugError
                ...
                : html`<span class="field-hint"
                    >${msg("Lowercase letters, numbers and hyphens", { id: "cad-hint-slug" })}</span
                  >`}
            </div>
            <div class="field">
          ...
          <div class="field">
            ...
            <input
              ...
              .value=${this._role}
              @input=${(e: Event) => {
                this._role = (e.target as HTMLInputElement).value;
              }}
            />
          </div>
        </div>
        ...
        <div class="section">
          ...
          ${this._providersLoading
            ? html`<span class="field-hint"
                >${msg("Loading providers...", { id: "cad-loading-providers" })}</span
              >`
            : html`
                <div class="field-row">
                  <div class="field">
                    <label for="agent-provider"
                      >${msg("Provider", { id: "cad-label-provider" })}</label
                    >
                    <select id="agent-provider" @change=${this._onProviderChange}>
                      ${this._providers.map(
                        (p) => html`
                          <option value=${p.id} ?selected=${this._selectedProvider?.id === p.id}>
                            ${p.label}
                          </option>
                        `,
                      )}
                  ...
                  <div class="field">
                    ...
                    >
                      ${(this._selectedProvider?.models ?? []).map(
                        (m) => html`
                          <option value=${m} ?selected=${this._model === m}>
                            ${m.split("/")[1] ?? m}
                          </option>
                        `,
                      )}
      ...
      <div class="dialog-footer">
        ...
        <button
          ...
          @click=${this._submit}
        >
          ${msg("Create agent", { id: "cad-btn-submit" })}
        </button>
      </div>
    `;
  }
