...
//
// Design notes:
// - openclaw start/restart call waitForHealth (30s timeout) — these return 500 in tests
//   since there is no real gateway running. We test that the instance is found (not 404)
//   and that commands are recorded in MockConnection.
// - stop works cleanly for both types (no health check required).
// - claw-runtime stop is a no-op when not running → 200.
...
describe("Lifecycle API", () => {
  ...
  beforeAll(async () => {
    ...
    // Seed an openclaw instance — stop/start/restart commands go through MockConnection
    ...
    // Seed a claw-runtime instance for stop tests (no health check, no daemon spawn)
    seedInstance(ctx.registry, serverId, {
      slug: "lifecycle-rt",
      port: 18811,
