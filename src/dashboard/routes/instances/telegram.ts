...
export function registerTelegramRoutes(app: Hono, deps: RouteDeps): void {
  ...
  app.get(
    "/api/instances/:slug/telegram/pairing",
    permission({
      action: ACTIONS.INSTANCE_TELEGRAM_PAIRING_READ,
  ...
  app.post(
    "/api/instances/:slug/telegram/pairing/approve",
    permission({
      action: ACTIONS.INSTANCE_TELEGRAM_PAIRING_APPROVE,
      resource: { kind: "instance", id: (c) => c.req.param("slug") },
    }),
    async (c) => {
      const { slug } = getInstanceContext(c);
      ...
      try {
        ...
        if (!currentConfig) {
          return apiError(
            ...
            400,
            "NO_CONFIG",
            "Runtime config not found — configure Telegram first",
          );
        }
  ...
  app.delete(
    "/api/instances/:slug/telegram/pairing/:code",
    permission({
